    ...
    >
      <a-row>
        <a-col v-for="(item,index) in effectValidateData" :key="index">
          ...
          >
            <a-textarea autosize style="height: auto" v-if="item.type =='textarea'"/>
            <a-input :placeholder="item.placeholder" v-if="item.type =='input'" disabled/>
            <a-select
              placeholder="请选择"
              v-if="item.type =='select'"
              @change="item.name == 'improvement_results' ? counts($event) : null"
            >
