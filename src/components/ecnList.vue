<template>
  <a-spin tip="加载中..." :spinning="spinning" class="spinning">
    <div id="MeetingRoom">
      <div class="title">
        <h4  @click="open = !open" style="height: 31px;margin-top: 0px;border: 1px solid #e6e6e6;">筛选</h4>
        <a-select v-model="templateChoose" @change="changeTemplate" 
                  style="width: 50%;border: none;" ghost>
          <a-select-option v-for="item in templates" :key="item.id">{{item.value}}</a-select-option>
        </a-select>
      </div>
      <section class="meeting-room-wrapper" v-if="showEhList">
        <div class="tips" v-show="ecnList.length === 0">无筛选结果</div>
          <div class="clearfix meeting-room-item" 
            v-for="(item, index) in ecnList" 
            :key="index">
            <div>
              <p class="info"><b>项目信息：</b>{{item.position_code}}</p>
              <p class="info"><b>异常编号：</b>{{item.project_exception_number}}</p>
              <p class="info"><b>异常处理状态：</b>{{item.question_status}}</p>
              <p class="infos"><b>异常问题描述：</b>{{item.theme}}</p>
              <p class="info"><b>计划出图时间：</b>{{item.drawing_time_required_time}}</p>
              ...
              <p style="text-align:right;margin-bottom: 0.3em;">
                <a-button type="primary" size="small"
                  ...
                  @click="editProblem(item)">编辑</a-button>
                ...
                <a-button size="small"  v-show="item.project_exception_number.indexOf('EHW')"
                v-if="((item.ehclose.indexOf(jobnumber)!=-1 || isClose) &&
                  ( item.question_status == '工程已出图' ||
                    item.question_status == '物料齐套' ||
                    item.question_status == '物料接收'))" @click="toClose(item)">关闭</a-button>
                <a-button type="primary" size="small" @click="goToECNDetail(item)">查看</a-button>
                <a-button type="primary" size="small" @click="goToECNList(item)" v-show="item.project_exception_number.indexOf('EHW')">清单</a-button>
                <a-button type="danger" ghost size="small" @click="del(item)"
                  v-show="item.question_status == '已接收异常' && isCanDel">删除</a-button>
                <a-button type="primary" size="small" @click="upload(item)" v-show="item.creator == jobnumber">上传/查看</a-button>
                <a-button size="small" @click="changePeople(item.id)"
                  v-show="item.question_status == '已接收异常' &&
          ...
          <a-pagination size="small" v-if="ecnList.length > 0" :pageSize="5" class="pagination" v-model="queryParams.page" :total="pageTotal" @change='changePage' />
      </section>
        ...
        <div class="tips" v-show="ecnList.length === 0">无筛选结果</div>
          <div class="clearfix meeting-room-item" 
            ...
            <div>
              ...
              <p class="info"><b>异常项目编号：</b>{{item.project_exception_number}}</p>
              ...
              <p style="text-align:right;margin-bottom: 0.3em;">
                ...
                <a-button type="primary" size="small" @click="checkEhwDetail(item)">查看
                </a-button>
              </p>
            </div>
          </div>
        ...
        <div class="tips" v-show="ecnList.length === 0">无筛选结果</div>
          <div class="clearfix meeting-room-item" 
            ...
            <div>
              ...
              <p class="info"><b>异常项目编号：</b>{{item.projectExceptionNumber}}</p>
              <p class="infos"><b>异常描述：</b>{{item.theme}}</p>
              <p class="info"><b>工位代码：</b>{{item.positionCode}}</p>
              <p class="info"><b>状态：</b>{{item.questionStatus}}</p>
              <p style="text-align:right;margin-bottom: 0.3em;">
                <a-button @click='editEHCProblem(item)' type="primary" size="small" 
                v-if="item.questionStatus != '关闭'">编辑</a-button>
                <a-button @click="checkEhcDetail(item)" type="primary" size="small">查看</a-button>
                <a-button @click="del(item)"
                :disabled="(root == 'admin') ? false : true"
                type="danger" ghost size="small" 
                v-if="item.questionStatus != '关闭' && item.questionStatus == '已接收问题'">删除</a-button>
                <a-button @click="changePeople(item.id)"
                :disabled="(root == 'admin' || item.ecrJobnum == jobnumber) ? false : true"
                size="small" 
                v-if="item.questionStatus != '关闭' && item.questionStatus == '已接收问题'">转交</a-button>
              </p>
            </div>
          </div>
          <a-pagination size="small" v-if="ecnList.length > 0" :pageSize="5" class="pagination" v-model="queryParams.page" :total="pageTotal" @change='changePage' />
          ...
          <a-row style="margin-bottom:20px;">
            <a-col>
              <a-radio-group v-model="queryParams.label">
                ...
                <a-radio value="myduty" v-if="showEhwList || showEhList">我的待办</a-radio>
              </a-radio-group>
            </a-col>
          </a-row>
          <a-row>
            <a-col>
              <a-form-item label="项目编号：">
                <a-input placeholder="项目标号搜索" v-model="queryParams.project_number" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col>
              <a-form-item label="异常编号：">
                <a-input placeholder="异常编号搜索" v-model="queryParams.project_exception_number" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col>
              <a-form-item label="异常问题描述">
                <a-textarea  placeholder="异常问题描述搜索" v-model="queryParams.theme" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col>
              <a-form-item label="工位代号">
                <a-textarea  placeholder="工位代号" v-model="queryParams.position_code" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col>
              <a-form-item label="开始时间：">
                <a-date-picker :allowClear="false" @change="startTimeChange"
          ...
          <a-row>
            <a-col>
              <a-form-item label="结束时间：">
                <a-date-picker :allowClear="false" @change="endTimeChange"
          ...
          <a-row>
            <a-col>
              <a-form-item label='状态：'>
                <a-select placeholder="请选择" allowClear v-model="queryParams.question_status">
                  <a-select-option v-for="item in option" :key="item.value">{{item.value}}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
          </a-row>
      ...
      <ehcClose v-if="ehcFlag" :closeExpand='closeExpand' :effectId='effectId'
        @getExpand='setCloseExpand' @getList='getecnList'>
      </ehcClose>
      <ehcReason v-if="ehcAnalysisFlag" :reasonExpand='reasonExpand' :effectId='effectId'
        @getExpand='setReasonExpand' @getList='getecnList'>
      </ehcReason>
      <!-- 删除弹窗 -->
      <a-modal title="删除" :visible="delVisible" @ok="sureDel" @cancel='cancelDel' okText='确定' cancelText='取消'>
        <a-form>
      ...
      <!-- 转交弹窗 -->
      <a-modal title="转交" :visible="changeEcrVisible"
        okText='确定' cancelText='取消' :confirmLoading='confirmLoading'
        @ok="sureChangeEcr" @cancel='changeEcrVisible = false'>
        <a-form :autoFormCreate="(form)=>{this.form = form}" layout="inline">
          <a-form-item label='转交人'
            fieldDecoratorId="username"
            :fieldDecoratorOptions="{rules: [{ required: true,message:'请填写转交人'}]}">
            <a-select
              showSearch
              placeholder="请选择转交人"
              style="width: 200px"
       ...
       <!-- 重置密码弹窗 -->
      <a-modal title="重置密码" :visible="resetPsdVisible"
        okText='确定' cancelText='取消' :footer="null"
        @ok="sureDel" @cancel='resetPsdVisible = false'
      >
        <a-form>
          <a-form-item label='重置密码'>
            <a-input-search placeholder="请输入工号"
              enterButton="确定" style="width:90%"
              @search="onSearch"
            />
          </a-form-item>
        </a-form>
      ...
      <div class="bottomtitle">
        ...
        <div class="flexbtn" v-if="isCanDel"
          @click="resetPsdVisible = true"
        >重置密码</div> 
      </div>
    </div>
  </a-spin>
...
<script>
  import {SELECTBYKEY,GETFORMDATA,SHOWEHWDATA,DELETEDATA,
    SEARCHNAME,CHANGEECRPRINCIPLE,RESETPWD,SHOWEHCDATA,DELETEEHC,
    CHANGEEHC} from '@/assets/js/url'
  ...
  export default {
    ...
    created() {
      ...
      this.nextTwoDate = moment(nextDate).add(1, "day").format("YYYY-MM-DD")
      ...
      this.isClose = root == 'userpmo' || group.indexOf('userpmo') != -1 ? true : false
    ...
    methods: {
      ...
      problemCreate(){
        ...
        dd.device.notification.actionSheet({
          ...
          otherButtons: ["改善问题（EH）", "外购件异常（EHW）", "程序异常处理（EHC）"],
      ...
      editProblem(item){
        ...
        if (this.jobnumber == item.creator || this.group.indexOf('userEHWQuestionEdit') != -1) {
        ...
        if (index != -1) {
          ...
          onSuccess (result) {
            if (result.buttonIndex == 0) {
              ...
              if (JSON.stringify(tempButtons) == JSON.stringify(["项目经理审核", "问题创建模块"])) {
                _this.$router.push({ 
                  path: 'ehwProblem', 
                  query: { id: item.id,
              ...
              } else if (JSON.stringify(tempButtons) == JSON.stringify(["项目经理审核", "问题创建模块", "IE工程师审核"])) {
                _this.$router.push({ 
                  path: 'ehwProblem', 
                  query: { id: item.id,
                    creator: item.creator,
              ...
              if (JSON.stringify(tempButtons) == JSON.stringify(["项目经理审核", "IE工程师审核", "问题创建模块"])) {
                _this.$router.push({ 
                  path: 'ehwProblem', 
                  query: { id: item.id,
                    creator: item.creator,
          ...
          dd.device.notification.actionSheet({
            title: "要编辑的模块？", //标题
            cancelButton: '取消', //取消按钮文本
            otherButtons: ["问题创建模块","原因分析模块"],
            onSuccess (result) {
              if (result.buttonIndex == 0) {
                _this.$router.push({path:'/createProblem',query:{
                  id: item.id,
                  creator: item.creator,
                  project_exception_number: item.project_exception_number,
                  isCounts: isCounts
                ...
                _this.$router.push({path: '/ecrModal', query:{id: item.id}})
              }
            },
            onFail: function(err) {}
          })
      ...
      editEHCProblem(record) {
        ...
        if ((record.inspectedArray && record.inspectedArray.split(',').indexOf(this.jobnumber) != -1)
          || this.root == 'admin' || this.group.indexOf('userEHCEffect') != -1) {
      ...
      sureDel(){
        if (this.delContent) {
          ...
          let value = {}
          if (!this.showEhcList) {
            this.axios.get(DELETEDATA,{params:{
              id : this.delId,
              label: 'outside',
            ...
            this.axios.post(DELETEEHC, qs.stringify({
              id: this.delId,
      ...
      onSearch(value){
        if(value){
          this.axios.post(RESETPWD,qs.stringify({
            jobnum : value,
            root: 'admin'
          })).then(res=>{
            if(res.data.success == true){
